...
contract("Lottery V2", ([alice, bob, carol, david, operator, treasury, injector]) => {
  ...
  describe("LOTTERY #1 - CUSTOM RANDOMNESS", async () => {
    ...
    it("David claims the jackpot", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 KLAY collected
      // 10053.0025 * (1-0.20) * 0.5 = 4021.201 KLAY
    ...
    it("Bob claims 10 winning tickets he bought", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 KLAY collected
      // 10053.0025 * (1-0.20) * 0.02 = 160.84804 KLAY
      // 10053.0025 * (1-0.20) * 0.03 = 241.27206 KLAY
