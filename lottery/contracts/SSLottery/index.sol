...
contract SSLottery is IndexedSSLottery {
    ...
    ) IndexedSSLottery(_roleControlAddress, _contractControlAddress, _minTicketPriceInUsd) {}
     ...
     */
    function changeRandomGenerator(address _vrfConsumerAddress) external onlyRole(RoleName.Owner) {
        if (currentLotteryId != 0) {
            requireClaimable(currentLotteryId);
        }

        // Request a random number from the generator based on a seed
        IVRFConsumer(_vrfConsumerAddress).requestRandomNumberDirect(thisAddress);
     ...
     */
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyRole(RoleName.Owner) {
     ...
     * @dev Only callable by owner
     */
    function setMaxNumberTicketsPerBuy(uint256 _maxNumberTicketsPerBuy) external onlyRole(RoleName.Owner) {
